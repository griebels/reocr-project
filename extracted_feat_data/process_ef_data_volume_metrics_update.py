...
def compute_hapax_token_fraction(word_counter):
    """
    Given a word_counter (Counter with words:counts) for a single 
    version of extracted features
    (e.g., EF20 or EF25), compute the fraction of word tokens 
    that belong to hapax types (i.e., words that appear only once).

    Returns: hapax_token_fraction
    """
    if not word_counter:
        return float("nan")
    ...
    #hapax tokens means count the words with freq == 1
    hapax_token_count = sum(1 for c in word_counter.values() if c == 1)

    return hapax_token_count / total_tokens if total_tokens else float("nan")
...
for fname in common_files:
    ...
    # getting fraction of "UNK" tags and delta change
    ...
    pos_unk_fraction_delta = (
    unk_frac_25 - unk_frac_20
    if not (pd.isna(unk_frac_20) or pd.isna(unk_frac_25))
    else float("nan")
)
    ...
    # hapax word fractions for EF2.0 and 2.5
    hapax_token_frac_20 = compute_hapax_token_fraction(words20)
    hapax_token_frac_25 = compute_hapax_token_fraction(words25)

    # hapax delta: EF25 - EF20
    hapax_token_fraction_delta = (
        hapax_token_frac_25 - hapax_token_frac_20
        if not (pd.isna(hapax_token_frac_20) or pd.isna(hapax_token_frac_25))
        else float("nan")
    )
    ...
    rows.append(
        {
            ...
            "pos_unk_fraction_delta": pos_unk_fraction_delta,
            ...
            # hapax word metrics
            "hapax_token_fraction_ef20": hapax_token_frac_20,
            "hapax_token_fraction_ef25": hapax_token_frac_25,
            "hapax_token_fraction_delta": hapax_token_fraction_delta,
